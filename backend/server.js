...
app.post("/api/checkout", authenticateToken, (req, res) => {
  ...
  db.serialize(() => {
    // Defensive: roll back any stuck/orphaned transaction before starting a fresh one.
    // This prevents "transaction is aborted" errors if a previous checkout crashed mid-flight.
    db.run("ROLLBACK", () => {
      db.run("BEGIN TRANSACTION");
    });
              ...
              db.run("COMMIT", (err3) => {
                ...
                items.forEach((item) => {
                  ...
                  db.get(
                    ...
                    [item.id],
                    (errProd, prodRow) => {
                      if (prodRow && prodRow.stock <= 5) {
                        const schema = getSchema();
                        const payload = {
                          title: "Low Stock Alert",
                          body: `${prodRow.name} has dropped to ${prodRow.stock} items.`,
                          type: "notice",
                          icon: "/favicon.png",
                          tag: "stock-" + item.id,
                          url: "/#sme-inventory",
                ...
                // Large POS Transaction Notification
                if (total_amount >= 100000) {
                  const schema = getSchema();
                  const payload = {
                    title: "Large POS Transaction",
                    body: `A POS order of UGX ${total_amount.toLocaleString()} was processed.`,
                    type: "notice",
                    icon: "/favicon.png",
                    tag: "large-pos-" + pos_order_id,
                    url: "/#sme-business",
                  };
                  socketPushToPermission("can_see_sme", payload);
              ...
              if (effectiveIsDelivery) {
                db.run(
                  ...
                  function (errDel) {
                    ...
                    // 🔔 Push notification: alert all users with Transport permissions of new delivery job
                    const schema = getSchema();
                    const deliveryPayload = {
                      title: "🚗 New Delivery Job!",
                      body: `Order for ${buyer_name || "Customer"} — ${delivery_address || "Location TBD"}. Tap to view.`,
                      type: "delivery",
                      icon: "/favicon.png",
                      badge: "/favicon.png",
                      tag: "delivery-" + pos_order_id,
                      requireInteraction: true,
                      url: "/#transport",
                    };
                    sendPushToPermission(
                      ...
                      console.error("[PUSH] Delivery push failed:", e.message),
                    );
                    socketPushToPermission(
                      "can_see_transport",
                      deliveryPayload,
                    );
                    completeCheckout();
...
// Receives queued offline sales from IndexedDB. Uses client_uuid for idempotency
// so retries never create duplicate orders. Returns the list of confirmed UUIDs
// so the client can safely delete only acknowledged records from its local queue.
              ...
              function (err2) {
                ...
                db.run("COMMIT", (err3) => {
                  ...
                  if (total_amount >= 100000) {
                    const schema = getSchema();
                    const payload = {
                      title: "Large POS Transaction (Sync)",
                      body: `A POS order of UGX ${total_amount.toLocaleString()} was synced.`,
                      type: "notice",
                      icon: "/favicon.png",
                      tag: "large-pos-" + pos_order_id,
                      url: "/#sme-business",
                    ...
                    function (err) {
                      ...
                      db.run("COMMIT", (err) => {
                        ...
                        sendPushToPermission(
                          ...
                          {
                            title: "COD Received",
                            body: `Order #${row.pos_order_id} COD payment of UGX ${row.total_amount} received.`,
                            type: "notice",
                            icon: "/favicon.png",
                            tag: "cod-" + row.pos_order_id,
                            url: "/#sme-business",
                          },
                          schema,
                        ).catch(() => {});
...
app.get("/api/pos/expected-cash", authenticateToken, (req, res) => {
    ...
    (err, shift) => {
      ...
      if (!shift)
        return res.json({
          ...
          dropped: 0,
          remaining_to_drop: 0,
          shift_open_time: null,
        });

      const shiftOpenTime = shift.open_time;

      // Sum all CASH income since shift opened
      db.get(
        `SELECT COALESCE(SUM(amount), 0) as total_cash
             FROM transactions
             WHERE type = 'INCOME'
               AND (description LIKE '%(CASH)%' OR description LIKE '%Cash%')
        ...
        (err2, incomeRow) => {
          ...
          // Sum all cash drops already done this shift
          db.get(
            `SELECT COALESCE(SUM(amount), 0) as total_dropped
                     FROM transactions
                     WHERE type = 'EXPENSE'
                       AND description = 'Cash Drop to Supervisor'
                       AND transaction_date >= ?`,
            [shiftOpenTime],
            (err3, dropRow) => {
              if (err3) return res.status(500).json({ error: err3.message });

              const expected = incomeRow.total_cash || 0;
              const dropped = dropRow.total_dropped || 0;
              const remaining_to_drop = Math.max(0, expected - dropped);

              res.json({
                expected,
                dropped,
                remaining_to_drop,
                shift_open_time: shiftOpenTime,
              });
            },
          );
        },
...
app.post("/api/pos/cash-drop", authenticateToken, (req, res) => {
  const allowed = ["Cashier", "CEO", "Supervisor"];
  if (!allowed.includes(req.user.role))
    return res
      .status(403)
      .json({ error: "Only cashiers can record a cash drop." });
  ...
  const desc = note
    ? `Cash Drop to Supervisor: ${note}`
    : "Cash Drop to Supervisor";

  // Step 1: Log the cash drop as a transaction
  db.run(
    ...
    [parsedAmount, desc, req.user.id],
    function (err) {
      if (err) return res.status(500).json({ error: err.message });

      const dropId = this.lastID;
      const dropAmountStr = parsedAmount.toLocaleString();

      // Step 2: Reset the supervisor's petty cash account —
      // Clear all expenses for this period and restore the full budget.
      db.get(
        `SELECT * FROM petty_cash_account ORDER BY id DESC LIMIT 1`,
        (err, acct) => {
          if (err || !acct) {
            // Even if the reset fails, still return success for the transaction
            console.error(
              "[CashDrop] Could not load petty cash account:",
            ...
            sendPushToRole(
              ...
              {
                title: "Cash Remitted",
                body: `Cashier ${req.user.name} has remitted UGX ${dropAmountStr} to you.`,
                type: "notice",
                icon: "/favicon.png",
                tag: "cashdrop-" + dropId,
                url: "/",
            ...
            return res.json({
              success: true,
              id: dropId,
              message: `Cash drop of UGX ${dropAmountStr} recorded successfully.`,
            });
          }

          const newBalance = acct.base_budget || 0;

          // Reset balance and period start date so the supervisor starts fresh
          db.run(
            `UPDATE petty_cash_account SET balance = ?, carried_balance = 0, last_reset_date = datetime('now') WHERE id = ?`,
            [newBalance, acct.id],
            (err2) => {
              if (err2)
                console.error(
                  "[CashDrop] Failed to reset petty cash account:",
                  err2.message,
                );

              // Also delete used expenses so they don't carry over
              db.run(`DELETE FROM petty_cash_expenses`, [], (err3) => {
                if (err3)
                  console.error(
                    "[CashDrop] Failed to clear petty cash expenses:",
                    err3.message,
                  );

                // Step 3: Send push notification to Supervisor
                const schema = getSchema();
                sendPushToRole(
                  "Supervisor",
                  {
                    title: "💰 Cash Received",
                    body: `You received UGX ${dropAmountStr} from ${req.user.name}.`,
                    type: "notice",
                    icon: "/favicon.png",
                    tag: "cashdrop-" + dropId,
                    url: "/",
                ...
                res.json({
                  success: true,
                  id: dropId,
                  message: `Cash drop of UGX ${dropAmountStr} recorded and petty cash reset successfully.`,
                });
              });
            },
          );
